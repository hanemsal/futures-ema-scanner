...
<div class="container">
    ...
    <form method="get" class="toolbar">
        <div class="filters">
            ...
            <button type="button" class="csv-btn" onclick="downloadCSV()">CSV İndir</button>
        </div>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ '%.2f'|format(s.vol_ratio_entry) if s.vol_ratio_entry is not none else '-' }}</td>
                        <td>{{ '%.2f'|format(s.vol_ratio_exit) if s.vol_ratio_exit is not none else '-' }}</td>
                        <td>{{ '%.2f'|format(s.body_ratio_entry) if s.body_ratio_entry is not none else '-' }}</td>
                        <td>{{ '%.2f'|format(s.body_ratio_exit) if s.body_ratio_exit is not none else '-' }}</td>
                        <td>{{ s.candle_type or '-' }}</td>
    ...
    <div class="mobile-cards">
        ...
        <div class="signal-card">
            ...
            <div class="signal-grid">
                ...
                <div><div class="k">Vol Entry</div><div>{{ '%.2f'|format(s.vol_ratio_entry) if s.vol_ratio_entry is not none else '-' }}</div></div>
                <div><div class="k">Vol Exit</div><div>{{ '%.2f'|format(s.vol_ratio_exit) if s.vol_ratio_exit is not none else '-' }}</div></div>
                <div><div class="k">Body Entry</div><div>{{ '%.2f'|format(s.body_ratio_entry) if s.body_ratio_entry is not none else '-' }}</div></div>
                <div><div class="k">Body Exit</div><div>{{ '%.2f'|format(s.body_ratio_exit) if s.body_ratio_exit is not none else '-' }}</div></div>
                <div><div class="k">Candle</div><div>{{ s.candle_type or '-' }}</div></div>
